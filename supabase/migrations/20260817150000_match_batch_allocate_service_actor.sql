-- match_batch_allocate: honor an explicit actor for service-role callers.
--
...
-- MCP-approved batch allocation returned BATCH_UNAUTHORIZED since
-- 20260601122000 deliberately dropped the old p_user_id argument. The web
-- /pending path only worked because it happens to carry a cookie session.
-- Reported via gnubok_feedback 2026-07-24 (codex/hermes) and 2026-08-06.
--
-- Fix: re-add p_user_id, but gated exactly like undo_sie_import
-- (20260727121000): the parameter is honored only when
-- auth.role() = 'service_role'; every other caller resolves from its own
-- auth.uid(), so an authenticated PostgREST caller cannot impersonate.
-- v_caller also stamps journal_entries.user_id and the payment rows, so
-- service-path commits are now attributed to the approving human instead
-- of failing outright.
--
-- The body is byte-for-byte the 20260801204551 definition (the latest:
-- payment-date paid_at) plus the header parameter and the actor-resolution
-- block. The 3-arg signature is dropped to avoid PostgREST overload
-- ambiguity; grants are re-asserted below because DROP discards them.
--
-- pg-test: tests/pg/match-batch-allocate-service-actor.pg.test.ts

DROP FUNCTION IF EXISTS public.match_batch_allocate(uuid, jsonb, uuid);
...
-- the fresh CREATE picks up Supabase default grants (PUBLIC + anon +
-- authenticated + service_role), so PUBLIC and anon are revoked explicitly
-- and the two legitimate callers are re-asserted.
...
COMMENT ON FUNCTION public.match_batch_allocate(uuid, jsonb, uuid, uuid) IS
  'Books one bank transaction against N invoices/supplier invoices in a single samlingsverifikat. p_user_id is honored only for service_role callers (the pending-operations commit path); every other caller resolves from its own auth.uid(). Not callable by anon.';
